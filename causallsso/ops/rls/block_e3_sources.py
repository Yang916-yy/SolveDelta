...
def _block_e3_sources_fwd_kernel(
    ...
    b_h = tl.load(h + vector_base + o_r, mask=m_r, other=0.0).to(tl.float32)
    b_q_raw = tl.load(q + vector_base + o_r, mask=m_r, other=0.0).to(tl.float32)
    ...
    edit_r = token_head * R
    b_key_raw = tl.load(keys + edit_r + o_r, mask=m_r, other=0.0).to(tl.float32)
    ...
    b_erase = tl.load(erase + edit_r + o_r, mask=m_r, other=0.0).to(tl.float32)
    ...
    o_v = tl.arange(0, BV)
    m_v = o_v < V
    raw_value = token_head * V + o_v
    b_value = tl.load(values + raw_value, mask=m_v, other=0.0).to(tl.float32)
    b_write = tl.load(write + raw_value, mask=m_v, other=0.0).to(tl.float32)
